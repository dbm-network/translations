...
subtitle: function(data) {
	const names = ['Komut Mesajı', 'Geçici Değişken', 'Sunucu Değişkeni', 'Evrensel Değişken'];
...
<div style="padding-top: 8px;">
	<div style="float: left; width: 35%;">
		...
		<select id="emoji" name="second-list" class="round" onchange="glob.onChange1(this)">
			<option value="4" selected>Emoji Yönelt</option>
			<option value="0">Özel Emoji</option>
			<option value="1">Geçici Değişken</option>
			<option value="2">Sunucu Değişkeni</option>
			<option value="3">Evrensel Değişken</option>
		</select>
	</div>
	<div id="varNameContainer2" style="float: right; width: 60%;">
		<span id="extName">Emoji  (Sağ-Tıkla -> Emojiyi Ekle)</span>:<br>
...
init: function() {
	...
	glob.onChange1 = function(event) {
		...
		if(value === 0) {
			...
			varNameInput.innerHTML = "Emoji  (Sağ-Tıkla -> Emojiyi Ekle)";
			document.getElementById('varNameContainer3').style.display = 'none';
			document.getElementById('varNameContainer2').style.display = null;
		} else {
