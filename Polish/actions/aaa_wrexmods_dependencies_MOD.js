...
WrexMODS.latest_changes = "Trzecia próba z instalowaniem modułu. Teraz rejestruję instalacje modułów";
	...
	return new Promise((resolve, reject) => {
		...
		try {
			...
			if(module.attempts >= this.MaxInstallAttemptsPerModule){
				console.error("DBM MODS (Node Module Installer v2.1): Nie można zainstalować automatycznie modułu " + moduleName + ". \n\n (Osiągnięto limit prób instalacji) \n Spróbuj uruchomić swojego bota za pomocą CMD (Ctrl + Shift + PPM (prawy przycisk myszy) [W systemie Windows], aby otworzyć okno CMD), następnie wpisz 'node bot.js' przynajmniej raz, aby umożliwić uruchomienie instalatora. \n Jeśli to się nie powiedzie, wpisz 'npm install --save " + moduleName + "' , następnie uruchom ponownie bota.");
				module.errored = true;
				reject(module);
			}else{

				try {			
					console.log("DBM MODS (Node Module Installer v2.1) Próba instalacji modułu: '" + moduleName + "'. Proszę czekać...\n");	
					...
					try {
						...
						console.log("DBM MODS (Node Module Installer v2.1) Moduł '" + moduleName + "' został zainstalowany. Możesz być zmuszony do ponownego uruchomienia bota.");	
						resolve(module)	;	

					} catch (error) {
						console.error("DBM MODS (Node Module Installer v2.1): Instalacja modułu  '" + moduleName + "' nie powiodła się. Liczba prób: " + module.attempts + " z " + this.MaxInstallAttemptsPerModule);
					...
					if(error.message.includes("Command failed")){
						console.log("DBM MODS (Node Module Installer v2.1): Moduł  '" + moduleName + "' nie istnieje!");
						console.error(error.message);
					}else{
						console.error("DBM MODS (Node Module Installer v2.1): GŁÓWNY BŁĄD: Zgłoś poniższe informacje do supportu DBM!");
	...
	return new Promise((resolve, reject) => {
		...
		try {
			...
			if(currentInstallAttempts >= this.MaxInstallAttempts){
				console.error("WrexMods: Nie można zainstalować automatycznie " + moduleName + ". (Osiągnięto limit prób instalacji) Spróbuj uruchomić swojego bota za pomocą CMD (Ctrl + Shift + PPM (prawy przycisk myszy) [W systemie Windows]), wpisz 'npm install --save" + moduleName + "' zanim przejdziesz dalej!");
				reject(false);
			}
					
			try {
				console.log("Instalowanoe modułu: " + moduleName);	
				...
				console.error("Could not automatically install " + moduleName + " Zainstaluj moduł ręcznie 'npm install " + moduleName + "' zanim kontynuujesz.");
				result = error;
			}
		}	  
