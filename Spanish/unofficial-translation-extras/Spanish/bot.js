...
//---------------------------------------------------------------------
// Bot
// Contiene funciones para controlar el bot
//---------------------------------------------------------------------

const Bot = DBM.Bot = {};

Bot.$cmds = {}; // Comandos normales
Bot.$icds = []; // Comandos que incluyen una palabra
Bot.$regx = []; // Comandos de Expresión Regular
Bot.$anym = []; // Comandos de Cualquier Mensaje
Bot.$evts = {}; // Eventos
...
Bot.onReady = function() {
	...
	console.log('Bot listo para ser usado!');
...
Actions.generateTimeString = function(miliSeconds) {
	...
	if(days > 0) {
		...
		times.push(days + (days === 1 ? " día" : " días"));
	}
	const hours = Math.floor(remaining / 60 / 60);
	if(hours > 0) {
		remaining -= (hours * 60 * 60);
		times.push(hours + (hours === 1 ? " hora" : " horas"));
	}
	const minutes = Math.floor(remaining / 60);
	if(minutes > 0) {
		remaining -= (minutes * 60);
		times.push(minutes + (minutes === 1 ? " minuto" : " minutos"));
	}
	const seconds = Math.floor(remaining);
	if(seconds > 0) {
		remaining -= (seconds);
		times.push(seconds + (seconds === 1 ? " segundo" : " segundos"));
	}
	...
	if(times.length === 1) {
		...
		result = times[0] + " y " + times[1];
	} else if(times.length === 3) {
		result = times[0] + ", " + times[1] + ", y " + times[2];
	} else if(times.length === 4) {
		result = times[0] + ", " + times[1] + ", " + times[2] + ", y " + times[3];
	}
	return result;
}
...
Actions.invokeActions = function(msg, actions) {
	...
	if(this.exists(act.name)) {
		...
		console.error(act.name + " no existe!");
	}
};
...
Actions.getErrorString = function(data, cache) {
	...
	return `Error con ${type} "${data.name}", Acción #${cache.index + 1}`;
};
...
Files.initStandalone = function() {
	...
	if(fs.existsSync(Actions.location)) {
		...
		console.error('Por favor copia la carpeta "acciones" de DBM a la carpeta de este bot\n' + Actions.location);
	}
};
