...
Actions.generateTimeString = function(miliSeconds) {
	...
	if(days > 0) {
		...
		times.push(days + (days === 1 ? " 天" : " 天"));
	}
	const hours = Math.floor(remaining / 60 / 60);
	if(hours > 0) {
		remaining -= (hours * 60 * 60);
		times.push(hours + (hours === 1 ? " 小時" : " 小時"));
	}
	const minutes = Math.floor(remaining / 60);
	if(minutes > 0) {
		remaining -= (minutes * 60);
		times.push(minutes + (minutes === 1 ? " 分" : " 分"));
	}
	const seconds = Math.floor(remaining);
	if(seconds > 0) {
		remaining -= (seconds);
		times.push(seconds + (seconds === 1 ? " 秒" : " 秒"));
	}
...
Files.startBot = function() {
	...
	if(typeof Actions.location === 'string' && fs.existsSync(Actions.location)) {
		...
		console.error('請從Discord Bot Maker目錄中將"Actions"資料夾複製到此bot目錄: \n' + Actions.location);
	}
};
