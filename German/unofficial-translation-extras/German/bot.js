...
Actions.invokeActions = function(msg, actions) {
	...
	if(this.exists(act.name)) {
		...
		console.error(act.name + " existiert nicht!");
	}
};
...
Actions.getErrorString = function(data, cache) {
	const type = data.permissions ? 'Befehl' : 'Event';
	return `Error mit ${type} "${data.name}", Aktion #${cache.index + 1}`;
};
...
Files.startBot = function() {
	...
	if(typeof Actions.location === 'string' && fs.existsSync(Actions.location)) {
		...
		console.error('Bitte kopiere den "Actions" Ordner aus dem Discord Bot Maker Verzeichnis in das Verzeichnis des Bots: \n' + Actions.location);
	}
};
	...
	for(let i = 0; i < max; i++) {
		...
		fs.readFile(filePath, function(error, content) {
			...
			try {
				...
				console.error(`Es gab ein Problem mit dem einlesen von ${this.dataFiles[i]}!`);
				return;
			}
